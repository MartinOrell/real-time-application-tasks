...
void Digit_Capsule::connectGui(int guiId){
    _guiId = guiId;
}
...
void Digit_Capsule::receiveRequestDigitMessage(){
    switch(_state){
        case Zero:
            sendDigitMessage(_guiId, 0);
            return;
        case One:
            sendDigitMessage(_guiId, 1);
            return;
        case Two:
            sendDigitMessage(_guiId, 2);
            return;
        case Three:
            sendDigitMessage(_guiId, 3);
            return;
        case Four:
            sendDigitMessage(_guiId, 4);
            return;
        case Five:
            sendDigitMessage(_guiId, 5);
            return;
        case Six:
            sendDigitMessage(_guiId, 6);
            return;
        case Seven:
            sendDigitMessage(_guiId, 7);
            return;
        case Eight:
            sendDigitMessage(_guiId, 8);
            return;
        case Nine:
            sendDigitMessage(_guiId, 9);
            return;
        default:
...
void Digit_Capsule::overflowTransition(){
    if(_overflowSendToId == -1){
        ...
        newDigit->connectGui(_guiId);
        newDigit->start();
        _capsuleRunnerPtr->addCapsule(std::move(newDigit));
        sendConnectToDigitMessage(_guiId, _id+1);
